(function() {
    ...
    angular.module('gantt.grDependencies').factory('GanttGroupDependencyTaskMouseHandler', ['$timeout', function($timeout) {
        var TaskMouseHandler = function(manager, task) {
            ...
            this.install = function() {
                if (!self.installed) {
                    self.hideEndpoints();
                    self.elementHandlers.push(new ElementHandler(self.task.row.$element));
