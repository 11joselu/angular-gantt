      ...
      function handleTaskConflict(conflictsList, task) {
        if (!(task.model.id in conflictsList) && task.$element) {
          try {
            ...
            angular.element(task.$element).addClass('gantt-task-conflict');
          }
        ...
        for (var i = 0, l = allTasks.length; i < l; i++) {
          ...
          if (task && !(task.model.id in conflictsList) && task.$element) {
            try {
              ...
              angular.element(task.$element).removeClass('gantt-task-conflict');
            }
          }
        }
      }
    ...
    var TaskMouseHandler = function(manager, task) {
      ...
      this.install = function() {
        if (!self.installed) {
          ...
          var elem = self.task.getContentElement();

          if (elem) {
            self.elementHandlers.push(new ElementHandler(elem));
...
(function(){
  'use strict';
  angular.module('gantt.groups').controller('GanttGroupController', ['$scope', 'GanttTaskGroup', 'ganttUtils', '$timeout', function($scope, TaskGroup, utils, $timeout) {
    var updateTaskGroup = function() {
      ...
      if (enabledValue) {
        ...
        if ($scope.taskGroup) {
          $scope.gantt.api.groups.raise.change($scope.taskGroup);
        }
      ...
      if ($scope.taskGroup !== undefined) {
        if ($scope.taskGroup.tasks.indexOf(task) > -1) {
          ...
          if(!$scope.$$phase && !$scope.$root.$$phase) {
            $scope.gantt.api.columns.raise.refresh();
            //$scope.$digest();
          }
        } else {
    ...
    $scope.gantt.api.groups.on.revalidate($scope,  function() {
      if ($scope.taskGroup && $scope.taskGroup.model.children) {
        $timeout(function() {
          $scope.gantt.api.groups.raise.viewChange($scope.taskGroup);
        });
      }
...
(function() {
  ...
  angular.module('gantt').factory('GanttTaskGroup', ['ganttUtils', 'GanttTask', '$timeout', function(utils, Task, $timeout) {
    var TaskGroup = function(row, pluginScope) {
      ...
      self.ancestor = self.pluginScope.hierarchy.ancestors(self.row);

      self.tasks = [];
      self.overviewTasks = [];
      self.promotedTasks = [];
      self.row.groups = self.row.groups || [];
      ...
      if (self.showGrouping) {
        ...
        if (row.isDaily()) {
          var from  = moment(self.from).startOf('day');
          var to = moment(self.to).endOf('day');
          self.left = row.rowsManager.gantt.getPositionByDate(from);
          self.width = row.rowsManager.gantt.getPositionByDate(to) - self.left;
        }
      }
    };
    ...
    TaskGroup.prototype.createModel = function() {
      ...
      if (this.row.from) {
        ...
        if (this.row.model.data.isMilestonesGantt) {
          days = 0;
        }
...
angular.module('gantt.milestones.templates', []).run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('plugins/milestones/milestones.tmpl.html',
    '<div class="gantt-task-milestones" sly-repeat="milestone in task.model.milestones" sly-always-evaluate>\n' +
...
angular.module('gantt.groups.templates', []).run(['$templateCache', function($templateCache) {
  $templateCache.put('plugins/groups/taskGroup.tmpl.html',
    ...
    '         ng-if="taskGroup.showGrouping" ng-style="{\'left\': taskGroup.left + \'px\', \'width\': taskGroup.width + \'px\'}">\n' +
...
angular.module('gantt.labels.templates', []).run(['$templateCache', function($templateCache) {
  $templateCache.put('plugins/labels/labelsBody.tmpl.html',
    ...
    '        <div sly-repeat="row in gantt.rowsManager.visibleRows" sly-always-evaluate>\n' +
    '            <div gantt-row-label\n' +
  ...
  $templateCache.put('plugins/labels/labelsHeader.tmpl.html',
    ...
    '    <div ng-if="gantt.columnsManager.columns.length > 0 && gantt.columnsManager.headers.length > 0">\n' +
    '        <div sly-repeat="header in gantt.columnsManager.headers" sly-always-evaluate>\n' +
...
angular.module('gantt.table.templates', []).run(['$templateCache', function($templateCache) {
  $templateCache.put('plugins/table/sideContentTable.tmpl.html',
    ...
    '            <div ng-if="ganttHeaderHeight" class="gantt-row-label-header gantt-row-label gantt-table-row gantt-table-header-row">\n' +
    ...
    '            <div gantt-vertical-scroll-receiver>\n' +
    '                <div class="gantt-table-row" ng-repeat="row in gantt.rowsManager.visibleRows" ng-if="!row.model.data.removeFromPlan" ng-controller="TableColumnRowController">\n' +
...
angular.module('gantt.tree.templates', []).run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('plugins/tree/treeBody.tmpl.html',
    ...
    '                 ng-repeat="row in gantt.rowsManager.visibleRows" ng-if="!row.model.data.removeFromPlan">\n' +
    '                &nbsp;\n' +
    '            </div>\n' +
    '        </div>\n' +
    ...
    '                <li ng-repeat="row in rootRows" ng-if="!row.model.data.removeFromPlan" ui-tree-node\n' +
  ...
  $templateCache.put('plugins/tree/treeBodyChildren.tmpl.html',
    ...
    '            </a>\n' +
    '            <span gantt-row-label class="gantt-label-text"\n' +
    '                ng-class="::{\n' +
    ...
    '    <li ng-repeat="row in childrenRows" ng-if="!row.model.data.removeFromPlan" ui-tree-node>\n' +
